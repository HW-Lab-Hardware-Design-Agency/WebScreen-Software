...
- `lv_line_set_points` keeps up to 16 coordinate pairs per line and frees its storage with the line. Updating one line does not change another.
- `label_set_text` preserves string contents, including newlines and quotes, with a 255-byte text limit.
- Script files must be nonempty and no larger than 1 MiB. Execution and recursion budgets turn runaway JavaScript into recoverable errors; `/errors` reports them and `/load` retries a corrected app.
