...
# Separate cached objects when switching library roots, compilers or flags.
signature = hashlib.sha256('\n'.join([*flags, os.environ.get('CC', 'gcc'),
    os.environ.get('CXX', 'g++'), *(str(p) for p in sources)]).encode()).hexdigest()[:12]
...
def compile_source(item):
    index, source = item
    output = build / f'{signature}-{index}.o'
