...
static bool webscreen_line_set_points(lv_obj_t *line, const lv_point_t *points, uint32_t count) {
  if (!lv_obj_check_type(line, &lv_line_class) || count == 0 || count > 16) return false;
  auto *copy = (lv_point_t *)lv_obj_get_user_data(line);
  if (!copy) {
    copy = (lv_point_t *)lv_mem_alloc(16 * sizeof(*copy));
