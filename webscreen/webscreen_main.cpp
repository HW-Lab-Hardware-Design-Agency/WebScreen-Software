...
bool webscreen_setup(void) {    WEBSCREEN_DEBUG_PRINTLN("WebScreen v" WEBSCREEN_VERSION_STRING " initializing...");
    
    
    if (!initialize_hardware()) {        WEBSCREEN_DEBUG_PRINTLN("Hardware initialization failed");
        return false;
    }
    
    
    
    if (!initialize_storage()) {        WEBSCREEN_DEBUG_PRINTLN("Warning: Storage initialization failed, using fallback mode");
        g_use_fallback = true;
    }
    
    
    
    if (!g_use_fallback && !load_configuration()) {        WEBSCREEN_DEBUG_PRINTLN("Warning: Configuration load failed, using defaults");
        
    }
    
    
    
    if (!g_use_fallback && g_webscreen_config.wifi.enabled) {        if (!initialize_network()) {
    ...
    if (!start_runtime()) {        WEBSCREEN_DEBUG_PRINTLN("Runtime initialization failed - using fallback");
        g_use_fallback = true;
        if (!webscreen_runtime_start_fallback()) {            WEBSCREEN_DEBUG_PRINTLN("Fallback startup failed");
            return false;
        }
    }
...
void webscreen_loop(void) {    run_main_loop();
}

const char* webscreen_get_state(void) {    switch (g_app_state) {
...
bool webscreen_is_healthy(void) {    return g_system_healthy;
}


void webscreen_shutdown(void) {    WEBSCREEN_DEBUG_PRINTLN("Initiating graceful shutdown...");
...
static bool initialize_hardware(void) {    WEBSCREEN_DEBUG_PRINTLN("Initializing hardware...");
    ...
    if (!webscreen_hardware_init()) {        WEBSCREEN_DEBUG_PRINTLN("Error: Display initialization failed");
        return false;
    }
...
static bool initialize_storage(void) {    WEBSCREEN_DEBUG_PRINTLN("Initializing SD Card...");
...
static bool load_configuration(void) {    WEBSCREEN_DEBUG_PRINTLN("Loading configuration...");
    
    
    if (!SD_MMC.exists(WEBSCREEN_CONFIG_FILENAME)) {        WEBSCREEN_DEBUG_PRINTLN("Config file not found, using defaults");
        return false;
    }
    
    
    File configFile = SD_MMC.open(WEBSCREEN_CONFIG_FILENAME, FILE_READ);
    if (!configFile) {        WEBSCREEN_DEBUG_PRINTLN("Failed to open config file");
        return false;
    }
    ...
    if (error) {        WEBSCREEN_DEBUG_PRINTF("Config parse error: %s\n", error.c_str());
        return false;
    }
    
    
    if (doc["wifi"]["ssid"]) {        WEBSCREEN_STR_COPY(g_webscreen_config.wifi.ssid, 
                          doc["wifi"]["ssid"], sizeof(g_webscreen_config.wifi.ssid));
    }
    if (doc["wifi"]["password"]) {        WEBSCREEN_STR_COPY(g_webscreen_config.wifi.password, 
    ...
    if (doc["script_file"]) {        WEBSCREEN_STR_COPY(g_webscreen_config.script_file, 
...
static bool initialize_network(void) {    WEBSCREEN_DEBUG_PRINTLN("Initializing network...");
    
    if (strlen(g_webscreen_config.wifi.ssid) == 0) {        WEBSCREEN_DEBUG_PRINTLN("No WiFi SSID configured");
        return false;
    }
...
static bool start_runtime(void) {    WEBSCREEN_DEBUG_PRINTLN("Starting runtime...");
    
    
    if (g_use_fallback) {        WEBSCREEN_DEBUG_PRINTLN("Starting fallback application");
        g_app_state = WEBSCREEN_STATE_RUNNING_FALLBACK;
        return webscreen_runtime_start_fallback();
    }
    
    
    if (!SD_MMC.exists(g_webscreen_config.script_file)) {        WEBSCREEN_DEBUG_PRINTF("Script file not found: %s\n", g_webscreen_config.script_file);
    ...
    if (webscreen_runtime_start_javascript(g_webscreen_config.script_file)) {        g_app_state = WEBSCREEN_STATE_RUNNING_JS;
        return true;
    } else {        WEBSCREEN_DEBUG_PRINTLN("JavaScript runtime failed, using fallback");
        g_use_fallback = true;
        g_app_state = WEBSCREEN_STATE_RUNNING_FALLBACK;
        return webscreen_runtime_start_fallback();
    }
...
static void run_main_loop(void) {    
    ...
    switch (g_app_state) {        case WEBSCREEN_STATE_RUNNING_JS:
    ...
    if (!g_use_fallback && g_webscreen_config.wifi.enabled) {        webscreen_network_loop();
    }
...
static void handle_system_health(void) {    uint32_t now = WEBSCREEN_MILLIS();
    
    
    if (now - g_last_health_check > 30000) {        g_last_health_check = now;
        ...
        if (memory_usage > WEBSCREEN_MEMORY_WARNING_THRESHOLD) {            WEBSCREEN_DEBUG_PRINTF("Warning: High memory usage (%.1f%%)\n", memory_usage * 100);
            g_system_healthy = false;
        } else {            g_system_healthy = true;
        }
        
        
        if (now - g_last_stats_print > WEBSCREEN_STATS_REPORT_INTERVAL_MS) {            g_last_stats_print = now;
...
bool webscreen_load_config(const char* path, 
                          ...
                          bool &outMqttEnabled,
                          uint32_t &outBgColor,
                          uint32_t &outFgColor) {    WEBSCREEN_DEBUG_PRINTF("Loading configuration from: %s\n", path);
    
    File f = SD_MMC.open(path);
    if (!f) {        WEBSCREEN_DEBUG_PRINTLN("No JSON config file found");
        return false;
    }
    ...
    if (error) {        WEBSCREEN_DEBUG_PRINTF("Failed to parse JSON: %s\n", error.c_str());
        return false;
    }
